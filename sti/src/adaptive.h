...
inline T calculate_mean_thresh(const T *src, const int width, const int height, const int x,
                          const int y, const int size, const int con)
{
    const int half_size = size / 2;
    const int x_offset = x - half_size;
...
void adaptive_threshold_mean(sti::image<T> &src, sti::image<T> &dst, int size, int con)
{
    ...
    for (int y = 0; y < height; ++y)
    {
        for (int x = 0; x < width; ++x)
        {
            ...
            if (pix >= threshold) dst.data()[index] = pixel_max<T>();
            if (pix < threshold) dst.data()[index] = pixel_min<T>();
        }
    }
}
...
int get_threshold_yen(const sti::image<T> &src)
{
    ...
    for (it = 0; it < 256; it++) {
        //crit = -1.0 * ((P1_sq[it] * P2_sq[it])> 0.0 ? log(P1_sq[it] * P2_sq[it]) : 0.0) + 2.0 * ((P1[it] * (1.0 - P1[it]))>0.0 ? log(P1[it] * (1.0 - P1[it])) : 0.0);
