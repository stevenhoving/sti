...
static void apply_kernel(const sti::core::image<T> &src, sti::core::image<T> &dst, const K &kernel)
{
        ...
        for (int x = 0; x < src.width(); ++x)
        {
            ...
            for (int k = -half_kernel_size; k <= half_kernel_size; k++)
            {
                for (int j = -half_kernel_size; j <= half_kernel_size; j++)
                {
                    ...
                    sum = sum + kernel[j + kernel.size() / 2][k + kernel.size() / 2] * (float)src.data()[index];
                }
            }
            const int index = (y * src.width()) + x;
            float pixel = (kernel.factor * sum) + kernel.offset;

            pixel = detail::clamp<T>(pixel, detail::image::pixel_min_value<T>(), detail::image::pixel_max_value<T>());
