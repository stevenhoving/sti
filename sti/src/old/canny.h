...
template <typename T>
void convolution(const T *in, T *out, const float *kernel, const int nx, const int ny, const int kn,
 ...
 */
template <typename T>
void gaussian_filter(const T *in, T *out, const int nx, const int ny, const float sigma)
{
    ...
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++)
        {
            kernel[c] = std::exp(-0.5 * (std::pow((i - mean) / sigma, 2.0) + std::pow((j - mean) / sigma, 2.0))) /
...
template <typename T>
void canny_edge_detection(const sti::image<T> &in, sti::image<T> &out, const int tmin, const int tmax,
                          const float sigma)
{
    ...
    if (G == NULL || after_Gx == NULL || after_Gy == NULL || nms == NULL)
    {
    ...
    for (int i = 1; i < nx - 1; i++)
        for (int j = 1; j < ny - 1; j++)
        {
            ...
            const float dir = (float)(std::fmod(std::atan2(after_Gy[c], after_Gx[c]) + M_PI, M_PI) / M_PI) * 8;
