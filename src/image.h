...
public:
    ...
    explicit image(const uint8_t *data, const int width, const int height)
        ...
        , height_(height)
    {
        // \todo we miss a possible improvement here, because a resize like this will NULL/init the newly created elements.
        data_.resize(width * height);
    ...
    inline T *data() const {return const_cast<T *>(data_.data());}
    inline iterator_type begin() const {return data_.begin();}
    inline iterator_type end() const {return data_.end();}

    inline const int width() const {return width_;}
    inline const int height() const {return height_;}
    inline const int stride() const {return width_;} // \note atm the stride is equal to the width
private:
    int width_ = 0;
    int height_ = 0;
    value_type data_;
...
template <typename T>
T pixel_min() { return static_cast<T>(0.0); }
template <typename T>
T pixel_max() { return static_cast<T>(1.0); }
template <>
uint8_t pixel_min<uint8_t>() { return 0; }
template <>
uint8_t pixel_max<uint8_t>() { return 0xFF; }
