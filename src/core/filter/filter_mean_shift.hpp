...
template<typename T>
class mean_shift_filter : public sti::core::filter::ifilter<T>
{
public:
    ...
    virtual T operator()(const sti::core::image<T> &src, const int y, const int x) const override final
    {
        int width = src.width();
        int height = src.height();

        using value_type = sti::core::filter::clamp_type<T>;
